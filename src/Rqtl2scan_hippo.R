...
sub_cross<-subset(bxd,ind=keepidx)
rownames(sub_cross$pheno)<-sub_cross$pheno$ID
sub_cross$pheno<-sub_cross$pheno[,-1]


##generating a phenotype data file
library(mice)
library(lattice)
#write.csv(sub_cross$pheno,file="hippocampus-pheno.csv")
trait<-read.csv(file="hippocampus-pheno.csv")
end<-dim(trait)[2]
#check NAs
table(colSums(is.na(sub_cross$pheno[,-end])))
